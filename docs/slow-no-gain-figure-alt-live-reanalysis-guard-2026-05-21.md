...
Decision: `provisional_runtime_guard_kept_original50_pending`

This is safe enough to keep as a narrow runtime candidate because it preserves score/PAC visibility, does not lower any checkpoint floor, and did not trigger on the focused outside or original figure/alt controls. The runtime benefit is not yet repeat-guaranteed on `4683`; it fired on one no-gain route and correctly stayed parked on a stricter repeat. It is not a completed broad acceptance gate: a fresh original-50 deterministic validation is still required before claiming the behavior as fully accepted under the active goal.
