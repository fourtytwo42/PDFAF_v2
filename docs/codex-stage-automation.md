...
- continues through rejected stages by pivoting away from the rejected topic, auto-restoring tracked edits and newly created source files when the stage started clean, and stopping only when restore would risk pre-existing user changes;
