...
async function parseReport(path: string): Promise<ParsedReport> {
  ...
  const failedRows = files.filter((row: Row) => row.status !== 'ok');
...
function buildMarkdownSummary(root: string, rows: ParsedReport[]): string {
  ...
  lines.push('');
  lines.push(
    '| Batch | Started | Public mean | Public delta | Target | Public fail ratio | Protected mean (delta) | Protected worst category | Protected worst overall | Files | Status |',
  );
  lines.push('| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |');
  ...
  for (const row of rows) {
    ...
    const protectedFail =
      row.protectedAnalyzedCount !== null && row.protectedFailedCount !== null
        ? `${row.protectedFailedCount}/${row.protectedAnalyzedCount}`
        : 'n/a';
    ...
    lines.push(
      ...
      )} | ${formatNumber(row.protectedWorstOverallRegression)} | ${formatInt(row.publicFiles)} | ${status} (${protectedFail}) |`,
    );
  }
  ...
  for (const row of failing) {
    ...
    if (row.protectedAfterMean !== null && row.protectedAnalyzedCount !== null && row.protectedFailedCount !== null) {
      lines.push(
        `  - Protected analysis: ${row.protectedFailedCount}/${row.protectedAnalyzedCount} failed, mean ${formatNumber(
