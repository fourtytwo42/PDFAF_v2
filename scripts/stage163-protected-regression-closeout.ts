...
export interface Stage163ToolRow {
  toolName?: string;
  outcome?: string;
  source?: string;
  stage?: number;
...
interface Stage163RowReport {
  id: string;
  file: string | null;
  classification: Stage128Classification;
  reasons: string[];
  stage162AnnotationRetryFired: boolean;
  stage162AnnotationRetryRows: number[];
...
const DEFAULT_REFERENCE_RUN = 'Output/experiment-corpus-baseline/run-stage162-full-2026-04-30-r1';
const DEFAULT_TARGET_RUN = 'Output/experiment-corpus-baseline/run-stage163-target-protected-2026-04-30-r1';
const DEFAULT_OUT = 'Output/experiment-corpus-baseline/stage163-protected-regression-closeout-2026-04-30-r1';
const DEFAULT_IDS = ['long-4516', 'short-4214', 'short-4176', 'long-4683'];
const DEFAULT_REPEATS = 5;
const STAGE162_LARGE_ANNOTATION_OWNERSHIP_DEBT = 50;
const ANNOTATION_TOOLS = new Set(['repair_native_link_structure', 'tag_unowned_annotations']);
...
function usage(): string {
  return [
    'Usage: pnpm exec tsx scripts/stage163-protected-regression-closeout.ts [options]',
...
function parseArgs(argv: string[] = process.argv.slice(2).filter((arg, index) => !(index === 0 && arg === '--'))): {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (arg === '--reference-run' || arg === '--stage162-run') args.referenceRun = next;
...
function invariantNumber(details: JsonRecord, key: string): number | null {
  return num(asRecord(details['invariants'])[key]);
}

function isZeroDebtAnnotationNoEffect(tool: Stage163ToolRow): boolean {
  if (!ANNOTATION_TOOLS.has(tool.toolName ?? '') || tool.outcome !== 'no_effect') return false;
  const details = parseDetails(tool.details);
  if (details['note'] !== 'annotation_ownership_not_preserved') return false;
  return (
    invariantNumber(details, 'visibleAnnotationsMissingStructParentBefore') === 0 &&
    invariantNumber(details, 'visibleAnnotationsMissingStructParentAfter') === 0 &&
    invariantNumber(details, 'visibleAnnotationsMissingStructureBefore') === 0 &&
    invariantNumber(details, 'visibleAnnotationsMissingStructureAfter') === 0
  );
}

function isLargeAnnotationDebtAttempt(tool: Stage163ToolRow): boolean {
  if (!ANNOTATION_TOOLS.has(tool.toolName ?? '')) return false;
  if (tool.outcome !== 'applied' && tool.outcome !== 'no_effect') return false;
  const details = parseDetails(tool.details);
  return (
    (invariantNumber(details, 'visibleAnnotationsMissingStructureBefore') ?? 0) >= STAGE162_LARGE_ANNOTATION_OWNERSHIP_DEBT ||
    (invariantNumber(details, 'visibleAnnotationsMissingStructParentBefore') ?? 0) >= STAGE162_LARGE_ANNOTATION_OWNERSHIP_DEBT
  );
}

export function detectStage162AnnotationRetryRows(tools: Stage163ToolRow[] = []): number[] {
  const rows: number[] = [];
  const sawZeroDebtNoEffectByTool = new Set<string>();
  tools.forEach((tool, index) => {
    const toolName = tool.toolName ?? '';
    if (!ANNOTATION_TOOLS.has(toolName)) return;
    if (isZeroDebtAnnotationNoEffect(tool)) {
      sawZeroDebtNoEffectByTool.add(toolName);
      return;
    }
    if (sawZeroDebtNoEffectByTool.has(toolName) && isLargeAnnotationDebtAttempt(tool)) rows.push(index);
  });
  return rows;
}

export function detectStage162AnnotationRetryTimeline(tools: Stage163ToolRow[] = []): boolean {
  return detectStage162AnnotationRetryRows(tools).length > 0;
}
...
export function classifyStage163Row(input: {
  floorScore: number | null;
...
async function buildRowReport(input: {
  ...
  const classification = classifyStage163Row({
    floorScore,
  ...
  const stage162AnnotationRetryRows = detectStage162AnnotationRetryRows(input.targetRow?.appliedTools ?? []);
  return {
    id: input.id,
    file: input.targetRow?.file ?? input.referenceRow?.file ?? input.baselineRow?.file ?? null,
    classification: classification.classification,
    reasons: classification.reasons,
    stage162AnnotationRetryFired: stage162AnnotationRetryRows.length > 0,
    stage162AnnotationRetryRows,
    ...
    reference: {
      ...
      categoryDeltas: categoryDeltas(baselineCategories, referenceCategories, 'reference'),
    },
    target: {
...
function renderMarkdown(report: {
  ...
  referenceRun: string;
  targetRun: string;
  rows: Stage163RowReport[];
}): string {
  const lines = [
    '# Stage 163 Protected Regression Closeout',
    '',
    `- Baseline: \`${report.baselineRun}\``,
    `- Reference run: \`${report.referenceRun}\``,
    `- Target run: \`${report.targetRun}\``,
    '',
    '| Row | Reference effective | Target after/reanalysis | Final repeat scores | Checkpoints | Stage162 retry | Local font | Classification | Reasons |',
    '| --- | ---: | --- | --- | ---: | --- | --- | --- | --- |',
  ];
  for (const row of report.rows) {
    lines.push([
      ...
      row.stage162AnnotationRetryFired ? `yes (${row.stage162AnnotationRetryRows.join(',')})` : 'no',
  ...
  for (const row of report.rows) {
    ...
    lines.push(`- Stage 162 annotation retry fired: ${row.stage162AnnotationRetryFired ? `yes (${row.stage162AnnotationRetryRows.join(',')})` : 'no'}`);
    ...
    if (row.reference.categoryDeltas.length > 0) {
      lines.push(`- Reference protected deltas: ${row.reference.categoryDeltas.slice(0, 8).map(delta => `${delta.key}:${delta.baseline}->${delta.reference}`).join(', ')}`);
    }
...
async function main(): Promise<void> {
  ...
  await writeFile(join(out, 'stage163-protected-regression-closeout.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
  await writeFile(join(out, 'stage163-protected-regression-closeout.md'), renderMarkdown(report), 'utf8');
