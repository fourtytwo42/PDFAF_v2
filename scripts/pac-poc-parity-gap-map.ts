...
export const PAC_POC_PARITY_LANES: PacPocParityLane[] = [
  {
    ...
    currentState: 'PAC-like table/header detection is strong and score-active. A narrow report-scale object-backed table proof is accepted for stable /Table targets with heavy header-association debt, while broader dense row-band and non-table-target shapes remain parked.',
    nextAction: 'Treat the report-scale object-backed Stage180 admission as the accepted table baseline. Reopen table remediation only with a new stable /Table target proof; keep dense row-band evidence supporting/diagnostic only.',
    acceptanceGate: 'No new table routing without verified /Table target refs, final table/PAC debt reduction, false_positive_applied=0, controls stable, no new hard timeout, and original-50 deterministic validation.',
  },
  {
