...
export function allInput0316TabOrderRouteGuardDecision(input: {
  ...
  const matchingTabOrder = input.stageApplied.some(row =>
    ...
    row.outcome === 'applied' &&
    replayStateSignatureBefore(row) === ALL_INPUT_0316_TAB_ORDER_ROUTE_SIGNATURE
  );
  if (!matchingTabOrder) return { reject: false, reason: null };
  const scoreMoved = (key: CategoryKey): boolean => {
    const before = categoryScore(input.before, key);
    const after = categoryScore(input.after, key);
    return before != null && after != null && after > before;
  ...
  return { reject: true, reason: ALL_INPUT_0316_TAB_ORDER_ROUTE_REASON };
}
...
function stageTargetsCategory(stageApplied: AppliedRemediationTool[], key: CategoryKey): boolean {
  ...
  if (key === 'reading_order') {
    return tools.has('repair_native_reading_order') ||
      ...
      tools.has('create_structure_from_degenerate_native_anchor') ||
      tools.has('synthesize_ocr_page_shell_reading_order_structure') ||
      tools.has('synthesize_basic_structure_from_layout');
  }
...
export function shouldRejectStageResult(input: {
  ...
  const pacGateDecision = (): { reject: boolean; reason: string | null; details?: string } => {
    ...
    const recovery = pacRuleUsefulRepairRecovery({
      ...
      beforeHeadingScore: categoryScore(input.before, 'heading_structure'),
      afterHeadingScore: categoryScore(input.after, 'heading_structure'),
      ...
      beforeReadingOrderScore: categoryScore(input.before, 'reading_order'),
  ...
  if (allInput0316RouteDecision.reject) {
    return {
      reject: true,
      reason: allInput0316RouteDecision.reason,
    };
  }
...
export function shouldTryLateParentLinksReadingOrderPostPass(input: {
  analysis: AnalysisResult;
  snapshot: DocumentSnapshot;
  appliedTools?: Array<Pick<AppliedRemediationTool, 'toolName' | 'outcome'>>;
}): boolean {
  ...
  if ((categoryScore(analysis, 'heading_structure') ?? 0) < 75) return false;
  if ((categoryScore(analysis, 'title_language') ?? 0) < 90) return false;
  if ((categoryScore(analysis, 'pdf_ua_compliance') ?? 0) < 79) return false;
...
async function applyGuardedPostPass(args: {
  ...
  if (pacGate.reject) {
    ...
    const recovery = pacRuleUsefulRepairRecovery({
      ...
      beforeHeadingScore: categoryScore(currentAnalysis, 'heading_structure'),
      afterHeadingScore: categoryScore(analyzed.result, 'heading_structure'),
      beforeLinkQualityScore: categoryScore(currentAnalysis, 'link_quality'),
      afterLinkQualityScore: categoryScore(analyzed.result, 'link_quality'),
      beforeReadingOrderScore: categoryScore(currentAnalysis, 'reading_order'),
...
const ALL_INPUT_PROPOSAL_BUFFER_SEQUENCE_TOOLS = new Set([
  'create_heading_from_candidate',
  'create_heading_from_tagged_visible_anchor',
  'repair_structure_conformance',
  'synthesize_basic_structure_from_layout',
...
async function tryAllInputProposalBufferSequence(args: {
  ...
  if (
    intermediateRegressions.length === 0 ||
    !intermediateRegressions.some(row => row.ruleId === 'pdfua.annotations.tagged_annotations_present') ||
    intermediateRegressions.some(row =>
      row.ruleId !== 'pdfua.annotations.tagged_annotations_present' &&
      row.ruleId !== 'pdfua.content.orphan_mcids_absent'
    )
  ) {
  ...
  const intermediateReading = categoryScore(args.analyzedState.analysis, 'reading_order');
  if (
    args.analyzedState.analysis.score <= args.stateBeforeStage.analysis.score ||
    beforeHeading == null ||
    intermediateHeading == null ||
    intermediateHeading <= beforeHeading ||
    intermediateReading == null ||
  ...
  for (const toolName of cleanupToolNames) {
    ...
    const beforeCleanupAnalysis = sequenceAnalysis;
    const beforeCleanupSnapshot = sequenceSnapshot;
    ...
    const cleanupRow: AppliedRemediationTool = {
      ...
      stage: args.stage.stageNumber,
      round: 1,
      scoreBefore: beforeCleanupAnalysis.score,
      scoreAfter: nextAnalysis.score,
      delta: nextAnalysis.score - beforeCleanupAnalysis.score,
      outcome: effectiveOutcome,
      details: enrichDetailsWithReplayState(JSON.stringify({
        ...
        beforeAnalysis: beforeCleanupAnalysis,
  ...
  const recovery = pacRuleStructureAnnotationSequenceRecovery({
    ...
    intermediateSnapshot: args.analyzedState.snapshot,
    finalSnapshot: sequenceSnapshot,
    toolNames: [structuralRow.toolName, ...cleanupRows.map(row => row.toolName)],
    beforeScore: args.stateBeforeStage.analysis.score,
    intermediateScore: args.analyzedState.analysis.score,
    finalScore: sequenceAnalysis.score,
    beforeHeadingScore: beforeHeading,
  ...
  structuralRow.details = JSON.stringify({
    outcome: 'applied',
    note: recovery.reason,
    originalDetails: parseMutationDetails(structuralRow.details) ?? structuralRow.details ?? null,
    sequenceRecovery: recovery.details ? JSON.parse(recovery.details) : null,
  });
  structuralRow.scoreAfter = sequenceAnalysis.score;
  structuralRow.delta = sequenceAnalysis.score - structuralRow.scoreBefore;
  enrichRowDetailsWithReplayState(structuralRow, {
...
async function finalizeAnalyzedStage(args: {
  ...
  if (rejectionDecision.reject) {
    ...
    if (allInputProposalBufferSequenceRecovered) {
      for (const row of stageApplied) {
        row.scoreAfter = allInputProposalBufferSequenceRecovered.analysis.score;
        row.delta = allInputProposalBufferSequenceRecovered.analysis.score - stageStartScore;
        enrichRowDetailsWithReplayState(row, {
          beforeAnalysis: stateBeforeStage.analysis,
          beforeSnapshot: stateBeforeStage.snapshot,
          afterAnalysis: allInputProposalBufferSequenceRecovered.analysis,
          afterSnapshot: allInputProposalBufferSequenceRecovered.snapshot,
        });
      }
...
async function applyOcrPageShellReadingOrderPostPass(args: {
  filename: string;
  signal?: AbortSignal;
  round: number;
  currentBuffer: Buffer;
  ...
  if (!shouldTryOcrPageShellReadingOrderRecovery(currentAnalysis, currentSnapshot)) {
    return { buffer: currentBuffer, analysis: currentAnalysis, snapshot: currentSnapshot, accepted: false };
  }
  const params = buildDefaultParams(toolName, currentAnalysis, currentSnapshot);
  const stageStarted = performance.now();
  ...
  const accepted = await applyGuardedPostPass({
    ...
    stage: 11,
    round,
    details: pythonMutationDetails(result, toolName) ?? 'stage144_ocr_page_shell_reading_order_structure',
    currentBuffer,
    currentAnalysis,
    currentSnapshot,
    nextBuffer: next,
...
async function applyLateParentLinksReadingOrderPostPass(args: {
  ...
  const { filename, signal, round, protectedBaseline } = args;
  const toolName = 'repair_top_level_parent_links';
  if (!shouldTryLateParentLinksReadingOrderPostPass({ analysis: currentAnalysis, snapshot: currentSnapshot, appliedTools })) {
    return { buffer: currentBuffer, analysis: currentAnalysis, snapshot: currentSnapshot, accepted: false };
  }
  const params = {
    ...buildDefaultParams(toolName, currentAnalysis, currentSnapshot, appliedTools),
    stage: 'late_finalization_parent_link_reading_order_topup',
  };
  const stageStarted = performance.now();
  const { buffer: next, result } = await runPythonMutationBatch(
    currentBuffer,
  ...
  const details = JSON.stringify({
    outcome: 'applied',
    note: 'late_finalization_parent_link_reading_order_topup',
    mutation: parseMutationDetails(mutationDetails) ?? mutationDetails ?? null,
  });
  const accepted = await applyGuardedPostPass({
    filename,
    ...
    appliedTools,
    runtimeSummary,
    tempPrefix: 'pdfaf-final-parent-links',
    protectedBaseline,
  });
  if (runtimeSummary) {
    pushStageTiming(runtimeSummary, {
...
async function applyIcjiaDocumentFinalization(args: {
  ...
  await tracePhase('late_parent_links_reading_order_topup', async () => {
    const repaired = await applyLateParentLinksReadingOrderPostPass({
      filename,
      signal,
      round,
      currentBuffer,
    ...
    currentBuffer = repaired.buffer;
    currentAnalysis = repaired.analysis;
...
export async function executePlaybook(
  ...
  if (stages.length === 0) {
    ...
    currentBuffer = ocrReading0.buffer;
    currentAnalysis = ocrReading0.analysis;
    currentSnapshot = ocrReading0.snapshot;
